        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="rounded-2xl bg-white p-6 ring-1 ring-ink-200">
            <div className="mb-4 flex items-center gap-3">
              <span className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-semibold ${step === 'info' ? 'bg-brand-600 text-white' : 'bg-ink-100 text-ink-500'}`}>1</span>
              <h2 className="font-display text-xl font-semibold text-ink-900">Contact & Shipping</h2>
            </div>
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input required placeholder="First name" value={form.firstName} onChange={update('firstName')} className="rounded-xl border border-ink-200 px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
                <input required placeholder="Last name" value={form.lastName} onChange={update('lastName')} className="rounded-xl border border-ink-200 px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
              </div>
              <input required placeholder="Street address" value={form.address} onChange={update('address')} className="w-full rounded-xl border border-ink-200 px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
              <div className="grid grid-cols-2 gap-4">
                <input required placeholder="City" value={form.city} onChange={update('city')} className="rounded-xl border border-ink-200 px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
                <input required placeholder="ZIP code" value={form.zip} onChange={update('zip')} className="rounded-xl border border-ink-200 px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
              </div>
            </div>
          </div>

          <div className={`rounded-2xl bg-white p-6 ring-1 ring-ink-200 transition-opacity ${step === 'payment' ? 'opacity-100' : 'pointer-events-none opacity-50'}`}>
            <div className="mb-4 flex items-center gap-3">
              <span className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-semibold ${step === 'payment' ? 'bg-brand-600 text-white' : 'bg-ink-100 text-ink-500'}`}>2</span>
              <h2 className="font-display text-xl font-semibold text-ink-900">Payment</h2>
            </div>
            <div className="grid gap-4">
              <div className="relative">
                <input required placeholder="Card number" value={form.cardNumber} onChange={update('cardNumber')} className="w-full rounded-xl border border-ink-200 px-4 py-3 pr-12 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
                <CreditCard size={20} className="absolute right-4 top-1/2 -translate-y-1/2 text-ink-400" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <input required placeholder="MM / YY" value={form.expiry} onChange={update('expiry')} className="rounded-xl border border-ink-200 px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
                <input required placeholder="CVC" value={form.cvc} onChange={update('cvc')} className="rounded-xl border border-ink-200 px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
              </div>
            </div>
          </div>
          ...
          >
            {step === 'info' ? 'Continue to Payment' : <><Lock size={16} /> Place Order — {formatPrice(total)}</>}
          </button>
        </form>
              ...
              {items.map((item) => (
                <div key={item.product.id} className="flex gap-3">
                  ...
                  <div className="flex flex-1 flex-col justify-center">
                    ...
                    <p className="text-xs text-ink-500">{formatPrice(item.product.price)} each</p>
                  </div>
